...
const static Json::StaticString FIELD_IS_NULLABLE("is_nullable");

FieldMeta::FieldMeta() : attr_type_(AttrType::UNDEFINED), attr_offset_(-1), attr_len_(0), visible_(false), field_id_(0) {}

FieldMeta::FieldMeta(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, int field_id, bool is_nullable)
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible, field_id, is_nullable);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}

RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible, int field_id, bool is_nullable)
{
...
bool FieldMeta::is_nullable() const { return is_nullable_; }
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  if (!is_nullable.isBool()) {
      LOG_ERROR("Is nullable field is not a bool value. json value=%s", is_nullable.toStyledString().c_str());
    return RC::INTERNAL;
  }
  ...
  bool        is_nullable_value = is_nullable.asBool();
  return field.init(name, type, offset, len, visible, field_id, is_nullable_value);
}
