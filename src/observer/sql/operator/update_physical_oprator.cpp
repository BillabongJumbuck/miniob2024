...
UpdatePhysicalOperator::UpdatePhysicalOperator(Table *table,const FieldMeta *field_meta, Expression* value)
{
  table_ = table;
...
RC UpdatePhysicalOperator::open(Trx *trx)
{
  ...
  Value value;
  // 求出value的实际值
  if(value_ -> type() == ExprType::VALUE) {
    ValueExpr *expr = dynamic_cast<ValueExpr *>(value_);
    value = expr->get_value();
  }else if(value_ -> type() == ExprType::SUBQUERY) {
    SubQueryExpr *subquery_expr = dynamic_cast<SubQueryExpr *>(value_);
  ...
  // 检查是否将null插入非null属性
  if(value.is_null() && !field_meta_->is_nullable()) {
  ...
  for (Record &record : records_) {
    rc = trx -> update_record(table_, record, value, field_meta_);
    if (rc != RC::SUCCESS) {
