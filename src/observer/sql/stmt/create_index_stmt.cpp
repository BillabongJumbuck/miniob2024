...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  for (auto &attr_name : create_index.attribute_name) {
    const FieldMeta *temp = table->table_meta().field(attr_name.c_str());
    if(temp == nullptr) {
      LOG_WARN("no such field in table. db=%s, table=%s, field name=%s",
               db->name(), table_name, attr_name.c_str());
      return RC::SCHEMA_FIELD_NOT_EXIST;
    }
    field_metas.push_back(const_cast<FieldMeta*>(temp));
  }
  ...
  stmt = new CreateIndexStmt(create_index.unique_tag, table, field_metas, create_index.index_name);
  return RC::SUCCESS;
}
