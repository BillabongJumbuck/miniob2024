...
RC PhysicalPlanGenerator::create_plan(PredicateLogicalOperator &pred_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  if(children_opers.empty()) {
    LOG_INFO("empty child oper, which means select no tables!");
    return RC::SUCCESS;
  }
...
RC PhysicalPlanGenerator::create_plan(OrderByLogicalOperator &logical_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  ...
  unique_ptr<OrderByPhysicalOperator> order_by_oper =
      make_unique<OrderByPhysicalOperator>(logical_oper.is_asc(), logical_oper.order_by_expressions());
